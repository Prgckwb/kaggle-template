...
- Lightning の `ModelCheckpoint` を使い、ファイル名に **exp / run / fold / epoch / スコアを全て入れる**:
  ...
  - ⚠ **メトリクス名にハイフンを使わない**（`macro_f1` と書き `macro-f1` と書かない）。
    区切りの `-` と区別できず、パーサが名前とスコアの境界を決められない。
    `docs/competition-profile.yaml` の `metric.name` もこの規約に従う
  - スコアは**負になり得る**（R2・相関係数）。命名も、名前から best を選ぶスクリプトも符号を落とさない
...
- **ckpt ファイルは上書きしない**: epoch とスコア入りの命名なので、
  ...
  - epoch もスコア（丸め後）も同一なら同名衝突は起こり得る。それは「同じ試行の焼き直し」なので上書きでよい
