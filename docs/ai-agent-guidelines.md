...
- 🔴 `git add -A` / `git add .` / `git add -u` — 相手の作業を巻き込んでコミットする。**常にパスを明示する**（`permissions.deny` と `.claude/hooks/guard.py` の二重で deny）
  - `-u`（`--update`）は未追跡ファイルこそ拾わないが**追跡中の全変更をステージする**ので危険度は同じ。`-Av` `-vu` のようにまとめた束も deny する
- 🔴 `git commit -a` / `git commit -am` / `git commit --all` — `git add -A` + commit と同じ。パスを明示して `git add` してから commit する（`--amend` は素通し）
